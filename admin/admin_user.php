...
if (isset($_POST['add_user'])) {
    ...
    $sql = "INSERT INTO tb_user (username, password, level) VALUES ('$username', '$password', '$level')";
    if ($config->query($sql)) {
...
if (isset($_POST['update_user'])) {
    ...
    $sql = "UPDATE tb_user SET username = '$username', password = '$password', level = '$level' WHERE id_user = '$id_user'";
    if ($config->query($sql)) {
        header("Location: admin_user.php");
    } else {
        echo "Error: " . $config->error;
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        echo "<tr>
                            ...
                            // Tampilkan password hanya untuk admin
                            if ($level == 'admin') {
                            ...
                            <td>
                                <button class='btn btn-warning btn-sm' data-toggle='modal' data-target='#editDataModal{$row['id_user']}'>Edit</button>
                                <a href='?delete={$row['id_user']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Hapus data ini?\")'>Hapus</a>
                            </td>
                        </tr>";
                        $no++;
                    ?>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="level" name="level" class="form-control" list="level-options"
                                value="<?= $row['level']; ?>" placeholder="Pilih level">
            ...
            <form method="POST">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <label for="level">Level</label>
                        <input type="text" id="level" name="level" class="form-control" list="level-options" 
                        value="<?php echo isset($row['level']) ? $row['level'] : ''; ?>" placeholder="Pilih level">
                        <datalist id="level-options">
                            <option value="admin"></option>
                            <option value="pimpinan"></option>
                            <option value="mahasiswa"></option>
