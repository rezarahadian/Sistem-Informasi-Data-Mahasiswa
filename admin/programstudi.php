...
$total_data_query = "SELECT COUNT(*) as total FROM tb_jurusan WHERE nama_jurusan LIKE '%$search%'";
...
$sql = "SELECT * FROM tb_jurusan WHERE nama_jurusan LIKE '%$search%' LIMIT $limit OFFSET $offset";
...
if (isset($_POST['add_jurusan'])) {
    ...
    // Pastikan variabel nama sesuai dengan nama input pada form
    $sql = "INSERT INTO tb_jurusan (nama_jurusan, jenjang) VALUES ('$nama_jurusan', '$jenjang')";
    if ($config->query($sql)) {
...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE tb_jurusan SET nama_jurusan='$nama_jurusan', jenjang='$jenjang' WHERE id_jurusan='$id_jurusan'";

    if (empty($nama_jurusan) || empty($jenjang)) {
        echo "<script>alert('Harap isi semua field!');</script>";
    } else {
        // Query untuk update data
        $query = "UPDATE tb_jurusan SET nama_jurusan='$nama_jurusan', jenjang='$jenjang' WHERE id_jurusan='$id_jurusan'";
        if (mysqli_query($config, $query)) {
            echo "<script>alert('Data berhasil diupdate!');</script>";
            echo "<script>window.location.href='programstudi.php';</script>";
        } else {
                ...
                <?php if ($result->num_rows > 0): ?>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editDataModal<?= $row['id_jurusan']; ?>">Edit</button>
                                <a href="?delete=<?= $row['id_jurusan']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data ini?')">Hapus</a>
                            </td>
                        </tr>
                        <!-- Modal Edit -->
                        <div class="modal fade" id="editDataModal<?= $row['id_jurusan']; ?>" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true">
                                ...
                                <form method="POST">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                <label>Jenjang</label>
                                                <input type="text" name="jenjang" class="form-control" value="<?= htmlspecialchars($row['jenjang']); ?>" required>
                                            </div>
                                        </div>
 ...
 <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="tambahDataModalLabel">Tambah Program Studi</h5>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label>Program Studi</label>
                            <input type="text" name="nama_jurusan" class="form-control" required placeholder="Masukan Program Studi">
                        </div>
                        <div class="mb-3">
                            <label>Jenjang</label>
                            <input type="text" name="jenjang" class="form-control" required placeholder="Masukan Jenjang">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" name="add_jurusan" class="btn btn-primary">Simpan</button>
