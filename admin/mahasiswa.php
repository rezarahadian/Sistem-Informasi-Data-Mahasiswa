...
// Query untuk mengambil data mahasiswa dengan pagination dan pencarian, serta LEFT JOIN dengan 
//m.*: Mengambil semua kolom dari tabel tb_mahasiswa dengan alias m.
//u.username: Mengambil kolom username dari tabel tb_user dengan alias u.
//left join berguna mengguhubungkan dua tabel tb_mahasiwa dan tb_user berdasarkan kolom id_user
$query = "
SELECT m.*, u.username
FROM tb_mahasiswa m
...
// Query untuk mengambil data id_user dan username dari tb_user
$query_user = "SELECT id_user, username FROM tb_user";
$result_user = mysqli_query($config, $query_user);
...
if (isset($_POST['tambah'])) {
    ...
    $email = $_POST['email'];
    $id_user = $_POST['id_user'];  // Menyimpan id_user yang diambil dari form

    // Query untuk menyimpan data mahasiswa
    $query_insert = "INSERT INTO tb_mahasiswa (nim, nama, id_jurusan, jenis_kelamin, tgl_lahir, alamat, no_telepon, email, id_user) 
                     VALUES ('$nim', '$nama', '$id_jurusan', '$jenis_kelamin', '$tgl_lahir', '$alamat', '$no_telepon', '$email', '$id_user')";
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars($mahasiswa['id_user']); ?></td>
                        <!-- untuk menampilkan data username yang berasal dari tabel tb_user (melalui JOIN dengan tabel tb_mahasiswa) pada kolom username.-->
                        <td><?= isset($mahasiswa['username']) ? htmlspecialchars($mahasiswa['username']) : 'Tidak Ada Username'; ?></td>
                        <td>
                            <!-- Tambahkan aksi edit dan hapus di sini -->
                            <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editDataModal<?= $mahasiswa['id_mahasiswa']; ?>">Edit</button>
                            <a href="mahasiswa.php?hapus=<?= $mahasiswa['id_mahasiswa']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Hapus</a>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenis_kelamin" class="form-control" required>
                            <option value="L" <?= $mahasiswa['jenis_kelamin'] == 'L' ? 'selected' : ''; ?>>L</option>
                            <option value="P" <?= $mahasiswa['jenis_kelamin'] == 'P' ? 'selected' : ''; ?>>P</option>
                        </select>
                    </div>
                    <div class="form-group">
        ...
        <div class="modal-content">
            ...
            <form action="mahasiswa.php" method="POST">
                <div class="modal-body">
                    ...
                    <select name="id_jurusan" class="form-control" required>
                        <?php 
                        // Pastikan query untuk mengambil nama jurusan dan jenjang
                        $query_jurusan = "SELECT id_jurusan, nama_jurusan, jenjang FROM tb_jurusan";
                        $result_jurusan = mysqli_query($config, $query_jurusan);

                        // Cek apakah ada hasil dari query
                        while ($jurusan = mysqli_fetch_assoc($result_jurusan)): ?>
                            <option value="<?= $jurusan['id_jurusan']; ?>">
                                <?= htmlspecialchars($jurusan['nama_jurusan'] . ' - ' . $jurusan['jenjang']); ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
